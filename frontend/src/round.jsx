...
export default function Round() {
  ...
  async function vote(winnerId) {
    ...
    navigate('/waiting', { state: { roundNumber: parseInt(roundNumber ?? '1', 10) } });
  }
    ...
    return (
      <div style={{ maxWidth: 480, margin: "60px auto", padding: "0 24px" }}>
        ...
        <p style={{ color: "var(--color-text-secondary)" }}>
          Your party may not have an active round yet, or your matchup could not be loaded.
        </p>
        <button
          onClick={() => navigate('/party')}
          style={{ marginTop: 20, padding: '12px 24px', borderRadius: 'var(--border-radius-md)' }}
        >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {[ideaA, ideaB].filter(Boolean).map((idea) => (
          <button
            ...
            onMouseEnter={(e) => {
              if (!voted) e.currentTarget.style.borderColor = "var(--color-border-primary)";
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.borderColor = "var(--color-border-tertiary)";
            }}
          >
            <div style={{ fontWeight: 500, fontSize: 16, marginBottom: 8, color: "var(--color-text-primary)" }}>
              {idea.title}
            </div>
